...
@task
def hotel_list_scraping_sapporo():
    hotel_results = []
    # Step 1. 크롬 웹브라우저 실행
    ...
    for i in range(1, 11):#(1, 11)
        for delta in range(i+1, 11):#(1, 11)
            for offset in offsets:
                ...
                url = "https://www.booking.com/searchresults.ko.html?ss=%EC%82%BF%ED%8F%AC%EB%A1%9C&ssne=%EB%82%98%EB%A6%AC%ED%83%80&ssne_untouched=%EB%82%98%EB%A6%AC%ED%83%80&label=ko-kr-booking-desktop-fDL93Zuo5ymosKSjAtXB9gS652828999093%3Apl%3Ata%3Ap1%3Ap2%3Aac%3Aap%3Aneg%3Afi%3Atikwd-324456682700%3Alp1009871%3Ali%3Adec%3Adm&aid=2311236&lang=ko&sb=1&src_elem=sb&src=searchresults&dest_id=-242395&dest_type=city&ac_position=0&ac_click_type=b&ac_langcode=ko&ac_suggestion_list_length=5&search_selected=true&search_pageview_id=11f78ac3a9420073&ac_meta=GhAxMWY3OGFjM2E5NDIwMDczIAAoATICa286CeyCv%2B2PrOuhnEAASgBQAA%3D%3D&checkin={}&checkout={}&group_adults=2&no_rooms=1&group_children=0&order=review_score_and_price&offset={}".format(checkin, checkout, offset)
                #url만 바꾸면 됨
    ...
    file_name = f'raw_data_hotel/sapporo/sapporo_{now}.json'
...
with DAG(
    dag_id='hotel_rawdata_dag_sapporo',
    ...
    # 직접 작성
    load_raw_data_hotel_sapporo = hotel_list_scraping_sapporo()