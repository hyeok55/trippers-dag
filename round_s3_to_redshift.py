...
def load_csv_to_redshift():
    ...
    csv_files = s3_hook.list_keys(bucket_name=s3_bucket_name, prefix='round_trip/')
    ...
    for csv_file in csv_files:
        ...
        redshift_hook.insert_rows(table='raw_data.flight_offers_round', rows=result_all.values.tolist(), target_fields=target_fields)



# DAG 정의
with DAG('round_s3_to_redshift', default_args=default_args, description='Transfer CSV files from S3 to Redshift', schedule_interval=timedelta(days=1),catchup = False) as dag:
    
    # CSV 파일을 레드시프트로 복사하는 태스크
    load_csv_to_redshift_task = PythonOperator(
